...
function fnGetReport_pass(res, flg) {
    if (res.split("|^|")[0] == "0") {
        ...
        if (res.split("|^|")[3] == "1") {
            ...
            $("#btnCopyINIT").attr("onclick", "fnFBImportPopup(0,1,2);");
        }
        else {
...
function fnShowClusterDetailPopupReadOnly(ctrl) {
    ...
    if (ClusterDetail != "") {
        ...
        strtable += "<thead>";
        strtable += "<tr>";
        strtable += "<th style='width:40px;'>#</th>";
        strtable += "<th style=''>Cluster</th>";
        strtable += "<th style='width:100px;'>Target</th>";
...
function fnShowClusterDetailPopupEditable(ctrl) {
    ...
    if ($(ctrl).closest("td[iden='Init']").prev().find("img[iden='ProductHier']").eq(0).attr("selectedstr") != "") {
        ...
        strtable += "<th style='width:100px'>Target</th>";
        strtable += "<th style='width:100px;'>COH No</th>";
        strtable += "<th style='width:150px;'>Sector</th>";
        ...
        if (ClusterDetail != "") {
            ...
            for (var i = 0; i < Arr.length; i++) {
                ...
                strtable += "<td iden='cluster-mapping'><input type='text' value='" + Arr[i].split("^")[2] + "' style='width:68px;'/><img src='../../Images/copy.png' title='Copy to All' style='width: 14px; margin-left: 5px;' onclick='fnClustermappingCopyCellWise(this);'/></td>";
            ...
            for (var i = 0; i < Arr.length; i++) {
                ...
                strtable += "<td iden='cluster-mapping'>" + (i + 1).toString() + "</td>";
                strtable += "<td iden='cluster-mapping'>" + Arr[i].split("|")[1] + "</td>";
                strtable += "<td iden='cluster-mapping'><input type='text' value='' style='width:68px;'/><img src='../../Images/copy.png' title='Copy to All' style='width: 14px; margin-left: 5px;' onclick='fnClustermappingCopyCellWise(this);'/></td>";
                ...
                strtable += "<td iden='cluster-mapping'><div style='position: relative;'><input type='text' iden='sector' style='width:116px;' value='' selectedstr='' title='' onkeyup='fnShowSectorPopup(this);' onclick='fnShowSectorPopup(this);' readonly='readonly'/><img src='../../Images/copy.png' title='Copy to All' style='width: 14px; margin-left: 5px;' onclick='fnClustermappingCopyCellWise(this);'/><div class='popup-sector'>" + $("#ConatntMatter_hdnSectorMstr").val() + "<div style='text-align: right; margin: 0.1rem 0.3rem;'><a class='btn btn-primary btn-small' onclick='fnHideSectorPopup();'>OK</a></div></div></div></td>";
                strtable += "</tr>";
            }
        }
...
function fnClustermappingCopyCellWise(ctrl) {
    ...
    if (cellIndex == 2) {
        var Tgt = $(ctrl).closest("td").find("input[type='text']").eq(0).val();
        $("#divClusterDetailPopup").find("table").eq(0).find("tbody").eq(0).find("tr[iden='cluster-mapping']").each(function () {
            $(this).find("td[iden='cluster-mapping']").eq(cellIndex).find("input[type='text']").eq(0).val(Tgt);
        });
    }
    else if (cellIndex == 3) {
...
function fnClustermappingCopytpAll(ctrl) {
    var Tgt = $(ctrl).closest("tr").find("td[iden='cluster-mapping']").eq(2).find("input[type='text']").eq(0).val();
    var COH = $(ctrl).closest("tr").find("td[iden='cluster-mapping']").eq(3).find("select[iden='coh']").eq(0).val();
    var Sector = $(ctrl).closest("tr").find("td[iden='cluster-mapping']").eq(4).find("input[type='text'][iden='sector']").eq(0).val();
    var Sectortitle = $(ctrl).closest("tr").find("td[iden='cluster-mapping']").eq(4).find("input[type='text'][iden='sector']").eq(0).attr("title");
    var SectorSelectedStr = $(ctrl).closest("tr").find("td[iden='cluster-mapping']").eq(4).find("input[type='text'][iden='sector']").eq(0).attr("selectedstr")

    $("#divClusterDetailPopup").find("table").eq(0).find("tbody").eq(0).find("tr[iden='cluster-mapping']").each(function () {
        $(this).find("td[iden='cluster-mapping']").eq(2).find("input[type='text']").eq(0).val(Tgt);
        $(this).find("td[iden='cluster-mapping']").eq(3).find("select[iden='coh']").eq(0).val(COH);
        $(this).find("td[iden='cluster-mapping']").eq(4).find("input[type='text'][iden='sector']").eq(0).val(Sector);
        $(this).find("td[iden='cluster-mapping']").eq(4).find("input[type='text'][iden='sector']").eq(0).attr("title", Sectortitle);
        $(this).find("td[iden='cluster-mapping']").eq(4).find("input[type='text'][iden='sector']").eq(0).attr("selectedstr", SectorSelectedStr);
    });
}

...
function fnDownload() {
    ...
    $("#ConatntMatter_hdnIncludeReleased").val("0");
    if ($("#chkReleased").is(":checked")) {
        $("#ConatntMatter_hdnIncludeReleased").val("1");
    }
    ...
    for (var i = 0; i < $("#ddlInitType option:selected").length; i++) {
        strInitType += $("#ddlInitType option:selected").eq(i).val() + "^";
    }
    $("#ConatntMatter_hdnINITType").val(strInitType);
    ...
    $("#ConatntMatter_hdnMSMPAlies").val(strUser);


    var flgMR = 0;
    if ($("#chkMR").is(":checked"))
        flgMR = 1;
    $("#ConatntMatter_hdnMR").val(flgMR);

...
function fnGetReport(flg) {
    ...
    if (LocString != "") {
        ...
        LocValues.push({ "col1": "0", "col2": "0", "col3": "6" });
    }
...
function fnGetReport_pass(res, flg) {
    if (res.split("|^|")[0] == "0") {
        ...
        if (res.split("|^|")[3] == "1") {
            ...
            $("#btnCopyINIT").attr("onclick", "fnFBImportPopup(0,2,2);");
        }
        else {
...
function fnAddNew() {
    if ($("#ConatntMatter_hdnIsNewAdditionAllowed").val() == "0") {
        ...
        strleft += "<td></td>";
        strleft += "<td></td>";
        strleft += "<td class='clstdAction'><img src='../../Images/save.png' title='Save' onclick='fnSave(this);'/><img src='../../Images/cancel.png' title='Cancel' onclick='fnCancel(this);'/></td>";
        if ($("#ConatntMatter_hdnRoleID").val() != "3" && $("#ConatntMatter_hdnRoleID").val() != "1015" && $("#ConatntMatter_hdnRoleID").val() != "4") {
        ...
        str += "<td iden='Init'></td>";
        str += "<td iden='Init'></td>";
        str += "<td iden='Init' class='clstdAction'><img src='../../Images/save.png' title='Save' onclick='fnSave(this);'/><img src='../../Images/cancel.png' title='Cancel' onclick='fnCancel(this);'/></td>";
        if ($("#ConatntMatter_hdnRoleID").val() != "3" && $("#ConatntMatter_hdnRoleID").val() != "1015" && $("#ConatntMatter_hdnRoleID").val() != "4") {
...
function fnEditCopy(ctrl, cntr) {
    ...
    if (flgRejectComment == "1")
        strbtn = "<img src='../../Images/save.png' title='Save' onclick='fnSave(this);'/><img src='../../Images/cancel.png' title='Cancel' onclick='fnCancel(this);'/><img src='../../Images/comments.png' title='Comments' onclick='fnGetRejectComment(this);'/>";
    else
        strbtn = "<img src='../../Images/save.png' title='Save' onclick='fnSave(this);'/><img src='../../Images/cancel.png' title='Cancel' onclick='fnCancel(this);'/>";
...
function fnSaveAllOpen(cntr) {
    ...
    if (SaveCntr != 0) {
        var ctrl = $("#tblleftfixed").find("tbody").eq(0).find("tr[flgEdit='1']").eq(cntr).find("td.clstdAction").eq(0).find("img[title='Save']").eq(0);

        fnSaveFocusBrand(ctrl, 0, cntr);
    }
    else {
...
function fnSave(ctrl) {
    SaveCntr = 1;
    fnSaveFocusBrand(ctrl, 0, 0);
}

function fnSaveFocusBrand(ctrl, flgOverwrite, cntr) {
    var lefttr = $(ctrl).closest("tr");
    var rowIndex = $(ctrl).closest("tr").index();
    var rttr = $("#tblReport").find("tbody").eq(0).find("tr[iden='Init']").eq(rowIndex);

    $("#tblleftfixed").find("tbody").eq(0).find("tr[flgActiveforSaving='1']").removeAttr("flgActiveforSaving");
    ...
    //}
        ...
        AutoHideAlertMsg("Please provide sector & COH no. for selected Cluster(s) !");
        return false;
    }
    else if (FromDate == "") {
        ...
        $("#dvloader").show();
        $(ctrl).closest("tr").attr("flgActiveforSaving", "1");
        PageMethods.fnSave(INITID, INITName, INITShortDescr, INITDescription, FromDate, ToDate, Bucket, BucketValues, LoginID, strLocation, strChannel + "|||" + copyChannelBucketID, IncudeLeap, IncudeSubD, Prod, UserID, flgSave, ClusterDetails, AllSKUArr, TopSKUArr, ClusterDetailsSector, flgOverwrite, fnSave_pass, fnfailed, cntr)
    }
}
function fnSave_pass(res, cntr) {
    if (res.split("|^|")[0] == "0") {
        ...
        $("#dvloader").hide();
        fnSoftWarning(res.split("|^|")[1], cntr);
    }
    else if (res.split("|^|")[0] == "5") {
...
function fnSoftWarning(msg, cntr) {
    $("#divConfirm").html(msg);
    $("#divConfirm").dialog({
        "modal": true,
        "width": "400",
        ...
        buttons: [{
            ...
            click: function () {
                ...
                var ctrl = $("#tblleftfixed").find("tbody").eq(0).find("tr[flgActiveforSaving='1']").eq(0).find("td.clstdAction").eq(0).find("img[title='Save']").eq(0);
                fnSaveFocusBrand(ctrl, "1", cntr);
            }
        },
        {
            text: 'No',
        ...
        buttons: [{
            ...
            click: function () {
                ...
                if (flgValidate == 0) {
                    ...
                    if ($(ctrl).attr("selectedstr") != selectedstr) {
                        $(ctrl).attr("selectedstr", selectedstr);


                        $(ctrl).closest("td").next().find("div[iden='content']").eq(0).html("");
                        $(ctrl).closest("td").next().find("img[iden='ProductHier']").eq(0).attr("selectedstr", "");
                    }
...
function fnShowClusterDetailPopupReadOnly(ctrl) {
    ...
    if (ClusterDetail != "") {
        ...
        for (var i = 0; i < Arr.length; i++) {
            ...
            strtable += "<td><select iden='coh' style='width:98%; padding: 2px;' defval='" + Arr[i].split("^")[3] + "' disabled>" + $("#ConatntMatter_hdnCOHMstr").val() + "</select></td>";
...
function fnShowClusterDetailPopupEditable(ctrl) {
    ...
    if ($(ctrl).closest("td[iden='Init']").prev().find("img[iden='ProductHier']").eq(0).attr("selectedstr") != "") {
        ...
        strtable += "<th style='width:100px;'>COH No</th>";
        strtable += "<th style='width:180px;'>Sector</th>";
        strtable += "<th style='width:50px;'>Action</th>";
        strtable += "</tr>";
        strtable += "</thead>";
        strtable += "<tbody>";
        ...
        if (ClusterDetail != "") {
            ...
            for (var i = 0; i < Arr.length; i++) {
                ...
                strtable += "<td iden='cluster-mapping'><select iden='coh' style='width:68px; padding: 2px;' defval='" + Arr[i].split("^")[3] + "'>" + $("#ConatntMatter_hdnCOHMstr").val() + "</select><img src='../../Images/copy.png' title='Copy to All' style='width: 14px; margin-left: 5px;' onclick='fnClustermappingCopyCellWise(this);'/></td>";
                ...
                strtable += "<td iden='cluster-mapping'><div style='position: relative;'><input type='text' iden='sector' style='width:146px;' value='" + strSector + "' selectedstr='" + Arr[i].split("^")[4] + "' title='" + strSector + "' onkeyup='fnShowSectorPopup(this);' onclick='fnShowSectorPopup(this);' readonly='readonly'/><img src='../../Images/copy.png' title='Copy to All' style='width: 14px; margin-left: 5px;' onclick='fnClustermappingCopyCellWise(this);'/><div class='popup-sector'>" + $("#ConatntMatter_hdnSectorMstr").val() + "<div style='text-align: right; margin: 0.1rem 0.3rem;'><a class='btn btn-primary btn-small' onclick='fnHideSectorPopup();'>OK</a></div></div></div></td>";
                strtable += "<td class='text-center'><img src='../../Images/copy.png' title='Copy to All' onclick='fnClustermappingCopytpAll(this);'/></td>";
                strtable += "</tr>";
            }
        }
        else {
            ...
            for (var i = 0; i < Arr.length; i++) {
                ...
                strtable += "<td iden='cluster-mapping'>" + (i + 1).toString() + "</td>";
                strtable += "<td iden='cluster-mapping'>" + Arr[i].split("|")[1] + "</td>";
                strtable += "<td iden='cluster-mapping'><select iden='coh' style='width:68px; padding: 2px;' defval='0'>" + $("#ConatntMatter_hdnCOHMstr").val() + "</select><img src='../../Images/copy.png' title='Copy to All' style='width: 14px; margin-left: 5px;' onclick='fnClustermappingCopyCellWise(this);'/></td>";

                strtable += "<td iden='cluster-mapping'><div style='position: relative;'><input type='text' iden='sector' style='width:146px;' value='' selectedstr='' title='' onkeyup='fnShowSectorPopup(this);' onclick='fnShowSectorPopup(this);' readonly='readonly'/><img src='../../Images/copy.png' title='Copy to All' style='width: 14px; margin-left: 5px;' onclick='fnClustermappingCopyCellWise(this);'/><div class='popup-sector'>" + $("#ConatntMatter_hdnSectorMstr").val() + "<div style='text-align: right; margin: 0.1rem 0.3rem;'><a class='btn btn-primary btn-small' onclick='fnHideSectorPopup();'>OK</a></div></div></div></td>";
                strtable += "</tr>";
            }
        }
        ...
        buttons: [{
            ...
            click: function () {
                ...
                $("#divClusterDetailPopup").find("table").eq(0).find("tbody").eq(0).find("tr[iden='cluster-mapping']").each(function () {
                    if (strValidateMsg == "") {
                        if ($(this).find("td[iden='cluster-mapping']").eq(2).find("select").eq(0).val() == "0") {
                            strValidateMsg = "Please select COH No. for Cluster - " + $(this).find("td[iden='cluster-mapping']").eq(1).html();
                        }
                        else if ($(this).find("td[iden='cluster-mapping']").eq(3).find("input").eq(0).val() == "") {
                            strValidateMsg = "Please select Sector(s) for Cluster - " + $(this).find("td[iden='cluster-mapping']").eq(1).html();
                        }
                        else {
                            selectedstr += "|" + $(this).attr("clusterId") + "^" + $(this).find("td[iden='cluster-mapping']").eq(1).html() + "^0^" + $(this).find("td[iden='cluster-mapping']").eq(2).find("select").eq(0).val() + "^" + $(this).find("td[iden='cluster-mapping']").eq(3).find("input").eq(0).attr("selectedstr");
                        }
                    }
                });
                if (selectedstr != "") {
...
function fnShowSectorPopup(ctrl) {
    fnHideSectorPopup();
    $(ctrl).parent().find("div.popup-sector").eq(0).show();
    if ($(ctrl).closest("tr[iden='cluster-mapping']").index() > 7)
        $(ctrl).parent().find("div.popup-sector").eq(0).css("top", "-" + $(ctrl).parent().find("div.popup-sector").eq(0).height() + "px");
    else
        $(ctrl).parent().find("div.popup-sector").eq(0).css("top", "20px");
    ...
    if ($(ctrl).attr("selectedstr") != "") {
        ...
        $(ctrl).parent().find("div.popup-sector").eq(0).find("table").eq(0).find("tr").each(function () {
...
function fnSelectSector(ctrl) {
    ...
    if (selectedstring != "") {
        $(ctrl).closest("table").parent().parent().find("input[type='text'][iden='sector']").eq(0).val(selectedstr.substring(2));
        $(ctrl).closest("table").parent().parent().find("input[type='text'][iden='sector']").eq(0).attr("title", selectedstr.substring(2));
        $(ctrl).closest("table").parent().parent().find("input[type='text'][iden='sector']").eq(0).attr("selectedstr", selectedstring.substring(1));
    }
    else {
        $(ctrl).closest("table").parent().parent().find("input[type='text'][iden='sector']").eq(0).val("");
        $(ctrl).closest("table").parent().parent().find("input[type='text'][iden='sector']").eq(0).attr("title", "");
        $(ctrl).closest("table").parent().parent().find("input[type='text'][iden='sector']").eq(0).attr("selectedstr", "");
    }
}
function fnClustermappingCopyCellWise(ctrl) {

    var cellIndex = $(ctrl).closest("td").index();
    if (cellIndex == 2) {
        var COH = $(ctrl).closest("td").find("select[iden='coh']").eq(0).val();
        $("#divClusterDetailPopup").find("table").eq(0).find("tbody").eq(0).find("tr[iden='cluster-mapping']").each(function () {
            $(this).find("td[iden='cluster-mapping']").eq(cellIndex).find("select[iden='coh']").eq(0).val(COH);
        });
    }
    else if (cellIndex == 3) {
        var Sector = $(ctrl).closest("td").find("input[type='text'][iden='sector']").eq(0).val();
        var Sectortitle = $(ctrl).closest("td").find("input[type='text'][iden='sector']").eq(0).attr("title");
        var SectorSelectedStr = $(ctrl).closest("td").find("input[type='text'][iden='sector']").eq(0).attr("selectedstr");
        $("#divClusterDetailPopup").find("table").eq(0).find("tbody").eq(0).find("tr[iden='cluster-mapping']").each(function () {
            $(this).find("td[iden='cluster-mapping']").eq(cellIndex).find("input[type='text'][iden='sector']").eq(0).val(Sector);
            $(this).find("td[iden='cluster-mapping']").eq(cellIndex).find("input[type='text'][iden='sector']").eq(0).attr("title", Sectortitle);
            $(this).find("td[iden='cluster-mapping']").eq(cellIndex).find("input[type='text'][iden='sector']").eq(0).attr("selectedstr", SectorSelectedStr);
        });
    }
}
function fnClustermappingCopytpAll(ctrl) {
    var COH = $(ctrl).closest("tr").find("td[iden='cluster-mapping']").eq(2).find("select[iden='coh']").eq(0).val();
    var Sector = $(ctrl).closest("tr").find("td[iden='cluster-mapping']").eq(3).find("input[type='text'][iden='sector']").eq(0).val();
    var Sectortitle = $(ctrl).closest("tr").find("td[iden='cluster-mapping']").eq(3).find("input[type='text'][iden='sector']").eq(0).attr("title");
    var SectorSelectedStr = $(ctrl).closest("tr").find("td[iden='cluster-mapping']").eq(3).find("input[type='text'][iden='sector']").eq(0).attr("selectedstr")

    $("#divClusterDetailPopup").find("table").eq(0).find("tbody").eq(0).find("tr[iden='cluster-mapping']").each(function () {
        $(this).find("td[iden='cluster-mapping']").eq(2).find("select[iden='coh']").eq(0).val(COH);
        $(this).find("td[iden='cluster-mapping']").eq(3).find("input[type='text'][iden='sector']").eq(0).val(Sector);
        $(this).find("td[iden='cluster-mapping']").eq(3).find("input[type='text'][iden='sector']").eq(0).attr("title", Sectortitle);
        $(this).find("td[iden='cluster-mapping']").eq(3).find("input[type='text'][iden='sector']").eq(0).attr("selectedstr", SectorSelectedStr);
    });
}
...
//    }
//}
function fnClusterPopupClearFilter() {
    $("#txtClusterPopupFilter").val("");
    $("#divClusterPopupTbl").find("table").eq(0).find("tbody").eq(0).find("tr").css("display", "table-row");
}
function fnClusterPopupFilter() {
    var Qtr = MonthArr.indexOf($("#ddlMonth").val().split("|")[0].split("-")[1]) + 1;
    var Yr = $("#ddlMonth").val().split("|")[0].split("-")[2];

    PageMethods.ClusterFilter("6", Qtr, Yr, $("#txtClusterPopupFilter").val(), ClusterFilter_pass, ClusterFilter_failed);
}
function ClusterFilter_pass(res) {
    if (res.split("|^|")[0] == "0") {
        $("#divClusterPopupTbl").find("table").eq(0).find("tbody").eq(0).find("tr").css("display", "none");

        var jsonTbl = $.parseJSON(res.split("|^|")[1]).Table;
        for (var i = 0; i < jsonTbl.length; i++) {
            $("#divClusterPopupTbl").find("table").eq(0).find("tbody").eq(0).find("tr[clusterid='" + jsonTbl[i]["ClusterID"] + "']").css("display", "table-row");
        }
    }
    else {
...
function ClusterFilter_failed(error) {
    AutoHideAlertMsg("Error  :  " + error);
}
function fnShowClusterPopup(ctrl, flg, IsImportPopup) {   // 1: filter, 2: In Tbl
    ...
    $("#divClusterPopup").dialog({
        ...
        open: function () {
            ...
            $(ctrl).attr("mth", Qtr);
            $(ctrl).attr("yr", Yr);
            PageMethods.GetClusters(LoginID, RoleID, UserID, "6", Qtr, Yr, GetClusters_pass, GetClusters_failed, selectedstr);
        },
        buttons: [{
            text: 'Select',
            class: 'btn-primary',

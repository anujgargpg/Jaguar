...
function fnEdit(ctrl) {
    ...
    $(ctrl).closest("tr").find("td[iden='creationdate']").eq(2).html("<select>" + $("#ConatntMatter_hdnMonthMstr").val() + "</select>");
    var option = $(ctrl).closest("tr").find("td[iden='creationdate']").eq(2).find("select").eq(0).find("option[value='" + Year + "|" + AddZero(Month) + "']").eq(0);
    var start = option.attr("start");
    var end = option.attr("end");

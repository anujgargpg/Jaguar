...
function fnShowHierFilter(ctrl) {
    ...
    if (flgVisibleHierFilter == "0") {
        ...
        if (RoleID == "1" || RoleID == "2" || RoleID == "4") {
            ...
            $("#divTypeSearchFilterBlock").css("width", "22%");
            if (RoleID == "2") {
        ...
        $("#divTypeSearchFilterBlock").css("width", "49%");
        if (RoleID == "2") {
            $("#divTypeSearchFilterBlock").css("width", "68%");
        }
...
function fnGetReport_pass(res, flg) {
    if (res.split("|^|")[0] == "0") {
        ...
        if (res.split("|^|")[3] == "1") {
            ...
            $("#btnImportFB").removeClass("btn-disabled");
            $("#btnImportFB").attr("onclick", "fnFBImportPopup(1,1,1);");
        }
        else {
            ...
            $("#btnImportFB").addClass("btn-disabled");
            $("#btnImportFB").removeAttr("onclick");
        }
...
function fnEditCopy(ctrl, cntr) {
    ...
    if (IncludeLeap == "1" && IncludeSubD == "1")
        tr.find("td[iden='Init']").eq(16).html("<input type='checkbox' checked/>Leap<br/><input type='checkbox' checked/>SubD");
    else if (IncludeLeap == "1")
        tr.find("td[iden='Init']").eq(16).html("<input type='checkbox' checked/>Leap<br/><input type='checkbox'/>SubD");
    else if (IncludeSubD == "1")
        tr.find("td[iden='Init']").eq(16).html("<input type='checkbox'/>Leap<br/><input type='checkbox' checked/>SubD");
    else
        tr.find("td[iden='Init']").eq(16).html("<input type='checkbox'/>Leap<br/><input type='checkbox'/>SubD");
...
function fnCancel(ctrl) {
    ...
    if (Init == "0") {
        ...
        if (IncludeLeap == "1" && IncludeSubD == "1")
            tr.find("td[iden='Init']").eq(16).html("Leap<br/>SubD");
        else if (IncludeLeap == "1")
            tr.find("td[iden='Init']").eq(16).html("Leap");
        else if (IncludeSubD == "1")
            tr.find("td[iden='Init']").eq(16).html("SubD");
        else
...
function fnSaveInitiative(ctrl, flgSave, cntr) {
    ...
    var IncudeLeap = 0;
    if (rttr.find("td[iden='Init']").eq(16).find("input[type='checkbox']").eq(0).is(":checked"))
        IncudeLeap = 1;
    var IncudeSubD = 0;
    if (rttr.find("td[iden='Init']").eq(16).find("input[type='checkbox']").eq(1).is(":checked"))
        IncudeSubD = 1;
...
function fnShowApplicationRulesPopup(ctrl, flgEdit) {
    ...
    if (strInit != "") {
        ...
        for (i = 1; i < ArrGrp.length; i++) {
            var active_grp_tr = $("#divAppRuleMixedCases").find("table").eq(0).find("tbody").eq(0).find("tr").eq(i - 1);
...
function fnShowApplicationRulesPopupWithoutMixedCases(ctrl, flgEdit) {
    ...
    if (strBase != "") {
        ...
        for (i = 1; i < ArrSlab.length; i++) {
            var active_slab = $("#divAppRuleBaseProdSec").find("div[iden='AppRuleSlabWiseContainer']").eq(i - 1);
            ...
            for (j = 1; j < ArrGrp.length; j++) {
                var active_grp = active_slab.find("table").eq(0).find("tbody").eq(0).find("tr").eq(j - 1);
    ...
    if (strInit != "") {
        ...
        for (i = 1; i < ArrGrp.length; i++) {
            var active_grp_tr = $("#divAppRuleBenefitSec").find("table").eq(0).find("tbody").eq(0).find("tr").eq(i - 1);
            active_grp_tr.attr("IsNewGrp", "0");
            active_grp_tr.attr("IsNewSlab", "0");
            active_grp_tr.attr("slabno", ArrGrp[i].split("*$*")[3]);
...
function GenerateAppRuleScript() {
    var strBase = ""; strInit = "";
    if ($("#divApplicationRulePopup").attr("flgMixedCases") == "1") {
...
function fnMRAccountPopup(ctrl, flgEdit) {
    if (flgEdit == 0) {
        $("#divPopup").dialog({
            "modal": true,
            "width": "490",
    ...
    str += "</div>";
    str += "<ul>";
    var json = $.parseJSON($("#ConatntMatter_hdnMRAccount").val());
    for (var i = 0; i < json.length; i++) {
        str += "<li strId='" + json[i].MRAccountID + "'><input type='checkbox' /><span>" + json[i].AccountName + "</span></option>";
    }
    str += "</ul>";
    $("#divPopup").html(str);

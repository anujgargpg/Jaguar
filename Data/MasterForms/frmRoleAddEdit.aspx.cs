...
public partial class MasterForms_frmRoleAddEdit : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            if (!IsPostBack)
            {
                ...
                if (Utilities.ValidateRoleBasedAccess(hdnRoleID.Value, "7"))
                    GetMaster();
                else
                    Response.Redirect("~/frmLogin.aspx");
    ...
    private static string CreateMstrTbl(DataTable dt, DataTable dtMenu, string[] SkipColumn, string tblname, string cls)
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            sbDescr.Clear();
            sb.Append("<tr RoleID='" + dt.Rows[i]["RoleId"] + "' RoleName='" + Utilities.XSSHandling(dt.Rows[i]["RoleName"].ToString()) + "' MenuID='" + MenuIDString + "' MenuDescr='" + Utilities.XSSHandling(MenuDescrString) + "'>");
            ...
            if (MenuDescrString.Length > 50)
                sb.Append("<td>" + Utilities.XSSHandling(MenuDescrString.Substring(0, 50)) + "...</td>");
            else
                sb.Append("<td>" + Utilities.XSSHandling(MenuDescrString) + "</td>");

            sb.Append("<td title='Edit Role'><img src='../../Images/edit.png' onclick='fnEdit(this);'/></td>");
            sb.Append("<td><img title='" + customTooltipforUserDetail(dt.Rows[i]["users"].ToString(), Utilities.XSSHandling(dt.Rows[i]["RoleName"].ToString())) + "' class='clsInform' src='../../Images/person.png' onclick='fnUserDeatils(this);'/></td>");
    ...
    public static string fnActive_DActiveUser(string UserID, string LoginID, string flgActiveValue)
    {
        ...
        if ((HttpContext.Current.Session["LoginID"] != null))
        {
            ...
            con = new SqlConnection(DBHelper.getDBConnectionString());
    ...
    public static string fnAddNewUser(string LoginID, string NodeID, string UserName, string Designation, string EmployeeCode, string EmailID, string PhoneNo, string UserStatus, string RoleID, object ArrTblData)
    {
        ...
        if ((HttpContext.Current.Session["LoginID"] != null))
        {
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("@EmpName", Utilities.XSSHandling(UserName));
                cmd.Parameters.AddWithValue("@Designation", Utilities.XSSHandling(Designation));
                cmd.Parameters.AddWithValue("@EmpCode", Utilities.XSSHandling(EmployeeCode));
                cmd.Parameters.AddWithValue("@PersonEmailID", Utilities.XSSHandling(EmailID));
    ...
    public static string fnSave(string RoleID, string RoleName, object CheckingMenuID, string strSelectedMenuID, string LoginID)
    {
        ...
        try
        {
            ...
            Scmd.Parameters.AddWithValue("@LoginID", LoginID);
            Scmd.Parameters.AddWithValue("@RoleID", RoleID);
            Scmd.Parameters.AddWithValue("@RoleName", Utilities.XSSHandling(RoleName));

...
public partial class SBDExtractInLeap : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            if (!IsPostBack)
            {
                ...
                if (Utilities.ValidateRoleBasedAccess(hdnRoleID.Value, "62"))
                    GetMaster();
                else
                    Response.Redirect("~/frmLogin.aspx");
            }
    ...
    [System.Web.Services.WebMethod()]
    public static string fnDownloadCSVExtractReport(string LoginID, string RoleID, string UserID, object objProd, object objLoc, object objChannel, string ProcessGroup, object objUser, string QtrNo, string QtrYear, string strVersion, string flgIncludeReleased, string ReleaseID)
    {
        try
        {
            // ----------- MS & P
            ...
            SqlCommand Scmd = new SqlCommand();
            Scmd.Connection = Scon;
            Scmd.CommandText = "spSBDExtractInLeapFormat";
            Scmd.Parameters.AddWithValue("@UserID", UserID);
            Scmd.Parameters.AddWithValue("@LoginID", LoginID);
            Scmd.Parameters.AddWithValue("@RoleID", RoleID);
            ...
            Scmd.Parameters.AddWithValue("@QtrNo", QtrNo);
            Scmd.Parameters.AddWithValue("@QtrYear", QtrYear);
            Scmd.Parameters.AddWithValue("@Version", Utilities.XSSHandling(strVersion));
            ...
            try
            {
                ...
                {
                    saveCSVFile(Ds.Tables[i + 1], SkipColumn, HttpContext.Current.Server.MapPath("~/Files/") + Ds.Tables[0].Rows[i]["strData"].ToString() + ".csv");

                    strFileName.Append(Ds.Tables[0].Rows[i]["strData"].ToString() + "|");
                }
            }
            catch (Exception ex)
            {

...
public partial class Data_EntryForms_InvoiceUpload : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            if (!IsPostBack)
            {
                ...
                if (Utilities.ValidateRoleBasedAccess(hdnRoleID.Value, "19"))
                    GetMaster();
                else if (Utilities.ValidateRoleBasedAccess(hdnRoleID.Value, "24"))
                    GetMaster();
                else
                    Response.Redirect("~/frmLogin.aspx");
            }
    ...
    [System.Web.Services.WebMethod()]
    public static string fnGetReport(string LoginID, object objProd, object objLoc, object objChannel, string FromDate, string ToDate, string UserID, object objMSMP)
    {
        try
        {

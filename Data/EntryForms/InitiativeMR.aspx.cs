...
public partial class _BucketMstr : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            if (!IsPostBack)
            {
                ...
                //if (Utilities.ValidateRoleBasedAccess(hdnRoleID.Value, "67"))
                //    GetMaster();
                //else
                //    Response.Redirect("~/frmLogin.aspx");
                GetMaster();
    ...
    {
        hdnProductLvl.Value = "<div class='producthrchy'>Product Level</div>" + Utilities.GetHierLvl("1", hdnLoginID.Value, hdnUserID.Value, hdnRoleID.Value, hdnNodeID.Value, hdnNodeType.Value, "67");

        hdnLocationLvl.Value = "<div class='producthrchy'>Location Level</div>" + Utilities.GetHierLvl("2", hdnLoginID.Value, hdnUserID.Value, hdnRoleID.Value, hdnNodeID.Value, hdnNodeType.Value, "67");

        hdnChannelLvl.Value = "<div class='producthrchy'>Channel Level</div>" + Utilities.GetHierLvl("3", hdnLoginID.Value, hdnUserID.Value, hdnRoleID.Value, hdnNodeID.Value, hdnNodeType.Value, "67");
    ...
    {    
        try
        {
            if (flg == "1")
            {
                ...
                if (tbl.Rows.Count > 0)
                {
                    return "0|^|" + Utilities.GetHierDetail("1", LoginID, UserID, RoleID, UserNodeID, UserNodeType, ProdLvl, "67") + "|^|" + Utilities.GetSelectedHierDetail("1", obj, "67");
                }
                else
                    return "0|^|" + Utilities.GetHierDetail("1", LoginID, UserID, RoleID, UserNodeID, UserNodeType, ProdLvl, "67") + "|^|";
            }
            else if (flg == "3")
                return "0|^|" + Utilities.GetHierTblAtSpecificLvl("1", LoginID, UserID, RoleID, UserNodeID, UserNodeType, ProdLvl, "67");
            else
                return "0|^|" + Utilities.GetParentHierForNewChild("1", LoginID, UserID, RoleID, UserNodeID, UserNodeType, ProdLvl, "67");
        }
        catch (Exception ex)
        {
            return "1|^|" + ex.Message;
    ...
    public static string fnLocationHier(string LoginID, string UserID, string RoleID, string UserNodeID, string UserNodeType, string ProdLvl, object obj, string InSubD)
    {
        try
        {
            ...
            if (tbl.Rows.Count > 0)
            {
                return "0|^|" + Utilities.GetHierDetail("2", LoginID, UserID, RoleID, UserNodeID, UserNodeType, ProdLvl, "67") + "|^|" + Utilities.GetSelectedHierDetail("2", obj, "67");
            }
            else
                return "0|^|" + Utilities.GetHierDetail("2", LoginID, UserID, RoleID, UserNodeID, UserNodeType, ProdLvl, "67") + "|^|";
        }
        catch (Exception ex)
        {
            return "1|^|" + ex.Message;
    ...
    public static string fnChannelHier(string LoginID, string UserID, string RoleID, string UserNodeID, string UserNodeType, string ProdLvl, object obj)
    {
        try
        {
            ...
            if (tbl.Rows.Count > 0)
            {
                return "0|^|" + Utilities.GetHierDetail("3", LoginID, UserID, RoleID, UserNodeID, UserNodeType, ProdLvl, "67") + "|^|" + Utilities.GetSelectedHierDetail("3", obj, "67");
            }
            else
                return "0|^|" + Utilities.GetHierDetail("3", LoginID, UserID, RoleID, UserNodeID, UserNodeType, ProdLvl, "67") + "|^|";
        }
        catch (Exception ex)
        {
            return "1|^|" + ex.Message;
    ...
    {
        return Utilities.GetSelectedHierDetail(BucketType, obj, "67");
    }

    [System.Web.Services.WebMethod()]
    public static string GetBucketbasedonType(string LoginID, string RoleID, string UserID, string BucketType)
    {
        return Utilities.GetBucketDetailsbasedonType(BucketType, LoginID, UserID, RoleID, "", "", false, "67");
    }
    ...
    [System.Web.Services.WebMethod()]
    public static string fnGetMRAccounts(object obj, string LoginID)
    {
        try
        {
            string str = JsonConvert.SerializeObject(obj, Formatting.Indented, new JsonSerializerSettings { ReferenceLoopHandling = ReferenceLoopHandling.Ignore });
            ...
            SqlCommand Scmd = new SqlCommand();
            Scmd.Connection = Scon;
            Scmd.CommandText = "spINITGetMRAccountofChannel";
            Scmd.CommandType = CommandType.StoredProcedure;
            Scmd.Parameters.AddWithValue("@Channels", tbl);
            //Scmd.Parameters.AddWithValue("@LoginID", LoginID);
            Scmd.CommandTimeout = 0;
            SqlDataAdapter Sdap = new SqlDataAdapter(Scmd);
            DataSet Ds = new DataSet();
            ...
            {
                return "0|^|" + JsonConvert.SerializeObject(Ds.Tables[0], Formatting.Indented, new JsonSerializerSettings { ReferenceLoopHandling = ReferenceLoopHandling.Ignore });
            }
            else
                return "1|^|<h4 style='color: #f00; padding: 10px;'>No MR Account Found !</h4>";
        }
        catch (Exception e)
        {
            return "2|^|<h4 style='color: #f00; padding: 10px;'>Error : " + e.Message + " !</h4>";
        }
    }
    ...
    public static string fnGetReport(string LoginID, string RoleID, string UserID, string FromDate, string ToDate, object objProd, object objLoc, object objChannel, string ProcessGroup, object objUser)
    {
        try
        {
            ...
            Scmd.Parameters.AddWithValue("@flgMR", "1");
            Scmd.CommandType = CommandType.StoredProcedure;
            Scmd.CommandTimeout = 0;
            Sdap = new SqlDataAdapter(Scmd);
            Ds = new DataSet();
    ...
    {        
        ...
        for (int j = 0; j < dt.Columns.Count; j++)
        {
            if (!SkipColumn.Contains(dt.Columns[j].ColumnName.ToString().Trim()))
            {
                if (dt.Columns[j].ColumnName.ToString().Trim() == "Code")
                {
                    ...
                    sb.Append("<th rowspan='2'>Include Account(s)</th>");
        ...
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                sbAccounts.Append(MRAccounts(dt.Rows[i]["INITID"].ToString(), Ds.Tables[5]));
                ...
                for (int j = 0; j < dt.Columns.Count; j++)
                {
                    if (!SkipColumn.Contains(dt.Columns[j].ColumnName.ToString()))
                    {
                        ...
                        {
                            if (sbAccounts.ToString() == "")
                                sb.Append("<td iden='Init'><a href='#' class='btn btn-primary btn-small' style='cursor: default;'>No Account Defined</a></td>");
                            else
                                sb.Append("<td iden='Init'><a href='#' class='btn btn-primary btn-small' selectedIds='" + sbAccounts.ToString() + "' flgEdit='0' onclick='fnMRAccountPopup(this, 0);'>View Details</a></td>");
                        }
            ...
            for (int j = 0; j < dt.Columns.Count; j++)
            {
                if (!SkipColumn.Contains(dt.Columns[j].ColumnName.ToString()))
                {
                    ...
                    else if (dt.Columns[j].ColumnName.ToString() == "Channel")
                    {
                        ...
                        sb.Append("<td iden='Init' iden_td='search' style='display: none;'></td>");
                    else
    ...
    private static string MRAccounts(string InitID, DataTable dt)
    {
        StringBuilder sb = new StringBuilder();
        ...
        if (dr.Length > 0)
        {
            ...
            {
                sb.Append(dttemp.Rows[i]["MRAccountID"].ToString() + "^");
            }
        }
        return sb.ToString();
    }

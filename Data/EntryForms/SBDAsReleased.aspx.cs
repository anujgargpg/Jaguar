...
public partial class _SBDMstr : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            if (!IsPostBack)
            {
                ...
                if (Utilities.ValidateRoleBasedAccess(hdnRoleID.Value, "45"))
                    GetMaster();
                else
                    Response.Redirect("~/frmLogin.aspx");
            }
    ...
    public static string fnGetReport(string LoginID, string RoleID, string UserID, object objProd, object objLoc, object objChannel, string ProcessGroup, object objUser, string QtrNo, string QtrYear)
    {
        try
        {
            ...
            SqlCommand Scmd = new SqlCommand();
            Scmd.Connection = Scon;
            Scmd.CommandText = "spSBDGetNormtoMarkReleased";
            Scmd.Parameters.AddWithValue("@UserID", UserID);
            Scmd.Parameters.AddWithValue("@LoginID", LoginID);
            Scmd.Parameters.AddWithValue("@RoleID", RoleID);
            ...
            return "0|^|" + CreateSBDApprovalTbl(RoleID, Ds, "tblReport", "clsReport") + "|^|" + CreateButtons(Ds.Tables[1], Ds.Tables[0].Rows.Count);
        }
        catch (Exception ex)
        {
            return "1|^|" + ex.Message;
    ...
    private static string CreateSBDApprovalTbl(string RoleID, DataSet Ds, string tblname, string cls)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                sb.Append("<tr iden='SBD' SBD='" + dt.Rows[i]["SBDID"] + "'>");
    ...
    private static string CreateButtons(DataTable dt, int SBDCntr)
    {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            if(SBDCntr > 0)
                sb.Append("<a href='#' class='btn btn-primary btn-sm' style='margin-right: 20px;' flgAction='" + dt.Rows[i]["ButtonID"].ToString() + "' onclick='fnSaveFinalAction(this);''>" + dt.Rows[i]["Button"].ToString() + "</a>");
            else

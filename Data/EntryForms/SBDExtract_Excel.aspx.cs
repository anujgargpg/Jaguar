    ...
    protected void btnDownload_Click(object sender, EventArgs e)
    {
        ...
        if (hdnLocationSelectedValue.Value != "")
        {
            for (var i = 0; i < hdnLocationSelectedValue.Value.Split('^').Length; i++)
            {
                ...
                dr[0] = hdnLocationSelectedValue.Value.Split('^')[i];
                dr[1] = "0";
                dr[2] = "4";
                tblLoc.Rows.Add(dr);
            }
        ...
        tblUser.Columns.Add("Col1", typeof(string));
        if (hdnUserSelectedValue.Value != "")
        {
            for (var i = 0; i < hdnUserSelectedValue.Value.Split('^').Length - 1; i++)
            {
                DataRow dr = tblUser.NewRow();
                dr[0] = hdnUserSelectedValue.Value.Split('^')[i];
                tblUser.Rows.Add(dr);
            }
        }
        ...
        Scmd.Parameters.AddWithValue("@LocSelection", tblLoc);
        Scmd.Parameters.AddWithValue("@ChannelSelection", tblChannel);
        Scmd.Parameters.AddWithValue("@ProcessGroup", "");
        Scmd.Parameters.AddWithValue("@Users", tblUser);
        Scmd.Parameters.AddWithValue("@QtrNo", hdnQuarterSelectedValue.Value.Split('|')[2]);
        Scmd.Parameters.AddWithValue("@QtrYear", hdnQuarterSelectedValue.Value.Split('|')[3]);
        SqlDataAdapter Sdap = new SqlDataAdapter(Scmd);
        Sdap.Fill(ds);

        if (ds.Tables[0].Rows.Count > 0)
        {
            string filename = "SBD Extract";
            string[] SkipColumn = new string[0];

            XLWorkbook wb = new XLWorkbook();
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                wb = AddWorkSheet(wb, ds.Tables[i + 1], SkipColumn, ds.Tables[0].Rows[i][0].ToString(), true, false);
            }
            try
            {

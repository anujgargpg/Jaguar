...
public partial class _FBApproval : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            if (!IsPostBack)
            {
                ...
                if (Utilities.ValidateRoleBasedAccess(hdnRoleID.Value, "73"))
                    GetMaster();
                else
                    Response.Redirect("~/frmLogin.aspx"); 
            }
    ...
    [System.Web.Services.WebMethod()]
    public static string fnGetReport(string LoginID, string RoleID, string UserID, string FBType, object objProd, object objLoc, object objChannel, string ProcessGroup, object objUser, string QtrNo, string QtrYear)
    {
        try
        {
            // ----------- MS & P
            ...
            Scmd.Connection = Scon;
            if(FBType == "1")
                Scmd.CommandText = "spFBGetNormtoMarkReleased_New";
            else
                Scmd.CommandText = "spFBGetNormtoMarkReleased_TopUp_New";
            Scmd.Parameters.AddWithValue("@UserID", UserID);
            Scmd.Parameters.AddWithValue("@LoginID", LoginID);
            Scmd.Parameters.AddWithValue("@RoleID", RoleID);
    ...
    private static string CreateSBDApprovalTbl(string RoleID, DataSet Ds, string tblname, string cls)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                sb.Append("<tr SBD='" + dt.Rows[i]["FBID"] + "'>");
    ...
    [System.Web.Services.WebMethod()]
    public static string fnSaveFinalAction(string RoleID, string LoginID, string UserID, string FBType, object objINIT)
    {
        try
        {

...
public partial class SBDExtractInLeap : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            if (!IsPostBack)
            {
                ...
                if (Utilities.ValidateRoleBasedAccess(hdnRoleID.Value, "44"))
                    GetMaster();
                else
                    Response.Redirect("~/frmLogin.aspx");
            }
    ...
    public static string ReleaseCycleFilter(string LoginID, string UserID, string RoleID, string SBDMonth, string SBDYear)
    {
        try
        {
            SqlConnection Scon = new SqlConnection(DBHelper.getDBConnectionString());
            SqlCommand Scmd = new SqlCommand();
            Scmd.Connection = Scon;
            Scmd.CommandText = "spSBDGetReleaseCycles";
            Scmd.Parameters.AddWithValue("@LoginID", LoginID);
            Scmd.Parameters.AddWithValue("@UserID", UserID);
            Scmd.Parameters.AddWithValue("@RoleID", RoleID);
            Scmd.Parameters.AddWithValue("@MOnthVal", SBDMonth);
            Scmd.Parameters.AddWithValue("@YearVal", SBDYear);
            Scmd.CommandType = CommandType.StoredProcedure;
            Scmd.CommandTimeout = 0;
            SqlDataAdapter Sdap = new SqlDataAdapter(Scmd);
            DataSet Ds = new DataSet();
            ...
            if (Ds.Tables[0].Rows.Count > 0)
            {
                ...
                for (int i = 0; Ds.Tables[0].Rows.Count > i; i++)
                    sb.Append("<option value='" + Ds.Tables[0].Rows[i]["ReleaseID"].ToString() + "'>" + Ds.Tables[0].Rows[i]["Descr"].ToString() + "</option>");

                return "0|^|" + sb.ToString();
            }
            else
                return "1|^|<option value='0'>No Records</option>";
        }
        catch (Exception ex)
        {
            return "1|^|<option value='0'>Error : " + ex.Message + "</option>";
        }
    }
    ...
    [System.Web.Services.WebMethod()]
    public static string fnDownloadCSVExtractReport(string LoginID, string RoleID, string UserID, object objProd, object objLoc, object objChannel, string ProcessGroup, object objUser, string QtrNo, string QtrYear, string flgIncludeReleased, string ReleaseID)
    {
        try
        {
            // ----------- MS & P
            ...
            SqlCommand Scmd = new SqlCommand();
            Scmd.Connection = Scon;
            Scmd.CommandText = "spSBDExtractCSV";
            Scmd.Parameters.AddWithValue("@UserID", UserID);
            Scmd.Parameters.AddWithValue("@LoginID", LoginID);
            Scmd.Parameters.AddWithValue("@RoleID", RoleID);
            ...
            Scmd.Parameters.AddWithValue("@QtrNo", QtrNo);
            Scmd.Parameters.AddWithValue("@QtrYear", QtrYear);
            Scmd.Parameters.AddWithValue("@flgIncludeReleased", flgIncludeReleased);
            Scmd.Parameters.AddWithValue("@ReleaseID", ReleaseID);
            Scmd.CommandType = CommandType.StoredProcedure;
            Scmd.CommandTimeout = 0;
            SqlDataAdapter Sdap = new SqlDataAdapter(Scmd);
            DataSet Ds = new DataSet();
            ...
            if (RoleID != "3")
            {
                ...
                HttpContext.Current.Session["dsSBD"] = Ds;
        ...
        {
            classSendMail.fnSendmail("saurav@astix.in", "", "Jaguar : Error while Downloading SBD Extract", ex.Message, "");
            return "1|^|" + ex.Message;
        }
    }
    ...
    {
        DataSet ds = (DataSet)HttpContext.Current.Session["dsSBD"];
        string filename = "SBD Extract dated " + DateTime.Now.ToString("dd-MMM-yyyy");
        ...
        for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            wb = AddWorkSheet(wb, ds.Tables[i + 1], SkipColumn, ds.Tables[0].Rows[i]["SheetName"].ToString());
